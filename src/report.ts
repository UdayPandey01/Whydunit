...
export function buildReport(scored: Scored[], support: Support): Report {
  ...
  const by_cause: Record<
    string,
    { n: number; amount: number; correct: number }
  > = {};
  ...
  for (const e of exceptions)
    for (const r of e.reasons) by_reason[r] = (by_reason[r] ?? 0) + 1;
...
export function renderAttribution(r: Report): string[] {
  ...
  for (const c of CAUSES) {
    ...
    if (b.n === 0) continue;
    lines.push(
      `  ${c.padEnd(22)} ${String(b.n).padStart(4)}  ${inr(b.amount).padStart(12)}  ${((100 * b.correct) / b.n).toFixed(0)}% correct`,
    );
  }
  lines.push('', 'Exception queue by reason (an exception may carry several):');
  for (const [reason, n] of Object.entries(r.by_reason).sort(
    (a, b) => b[1] - a[1],
  )) {
    lines.push(
      `  ${String(n).padStart(4)}  ${REASON_TEXT[reason as ExceptionReason]}`,
    );
  }
  lines.push('');
  return lines;
