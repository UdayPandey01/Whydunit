...
export const EFFECTFUL: ActionName[] = [
  'reschedule',
  'refire_notification_then_reschedule',
];
...
export function checkConstraints(
  ...
  record(
    CHECKS.MAX_INTERVENTIONS,
    effectful,
    ctx.interventions_used < MAX_INTERVENTIONS_PER_CYCLE,
  );
  ...
  record(
    ...
    plan.cause !== 'C4_CANCELLATION' &&
      (ctx.revoked_at === null ||
        (plan.scheduled_at !== null && plan.scheduled_at < ctx.revoked_at)),
  );

  const schedules = plan.scheduled_at !== null;
  record(
    CHECKS.NOT_RESTRICTED,
    schedules,
    schedules && !isRestrictedTime(plan.scheduled_at!),
  );
  ...
  record(
    ...
    schedules &&
      ...
      (plan.scheduled_at! - plan.notification_dispatch_at) / HOUR_MS >=
  ...
  return {
    ok,
    plan: ok ? (plan as CheckedPlan) : null,
    passed,
    failed,
    skipped,
  };
