...
export class SimulatedPsp implements PspClient {
  ...
  constructor(
    opts: { seed?: number; mandates?: number; horizonDays?: number } = {},
  ) {
    ...
    for (const list of this.notices.values())
  ...
  async fetchFailedDebits(since: Date): Promise<ObservedAttempt[]> {
    ...
    return this.observations.filter(
      (o) => !o.success && Date.parse(o.timestamp) >= from,
    );
  }
  ...
  async scheduleDebit(
    ...
    if (mandate === undefined)
      return FAILED('mandate_not_found', `unknown mandate ${mandateId}`);
    if (this.cancelled.has(mandateId))
      return FAILED('payment_cancelled', 'mandate cancelled');
    ...
    if (pending !== undefined) {
      ...
      const bank =
        this.records.find((r) => r.mandate_id === mandateId)?.bank ?? 'HDFC';
      const fresh = {
        dispatchMs,
        delivered: wasDeliveredByBank(
          bank,
          dispatchMs,
          makeRng(hash32(pending)),
        ),
      };
  ...
  async sendPreDebitNotification(
    ...
    if (!this.mandates.has(mandateId))
      return FAILED('mandate_not_found', `unknown mandate ${mandateId}`);
  ...
  private noticeGoverning(mandateId: string, atMs: number): Notify {
    ...
    for (const n of list) {
      if (
        n.dispatchMs <= atMs &&
        (best === null || n.dispatchMs > best.dispatchMs)
      )
        best = n;
    }
