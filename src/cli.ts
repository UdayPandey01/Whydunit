...
function summarise(
  ...
  ui.step(
    'ok',
    `${world.length.toLocaleString('en-IN')} payment attempts generated over ${HORIZON_DAYS} days`,
  );
  ui.step(
    'ok',
    `${fails.length.toLocaleString('en-IN')} failures emerged from the world processes`,
  );
  ...
  ui.kv([
    ...
    [
      'Failed attempts',
      bad(fails.length.toLocaleString('en-IN')) +
        dim(`  (${pct(fails.length, world.length)})`),
    ],
  ...
  for (const f of fails)
    byCause.set(f.cause!, (byCause.get(f.cause!) ?? 0) + 1);
  ...
  const withReceipt = observations.filter(
    (o) => o.notification.receipt !== null,
  ).length;
  const withRevoke = observations.filter(
    (o) => o.lifecycle_events.length > 0,
  ).length;
  const silentChurn = world.filter(
    (w) => w.world.churned_at !== null && !w.world.churn_emits_event,
  ).length;
...
function buildFeatures(): void {
  ...
  const explicitChurn = new Map(
    world.map((w) => [w.attempt_id, w.world.churn_emits_event]),
  );
  ...
  ui.step(
    'ok',
    `${rows.length.toLocaleString('en-IN')} failed payments prepared`,
  );
  ui.step(
    'ok',
    `${nFeatures} signals extracted per payment, none of them look-ahead`,
  );
  ...
  for (const scheme of ['mandate', 'bank', 'time'] as const) {
    ...
    const totalOf = (side: string) =>
      [...(counts.get(side) ?? new Map<string, number>()).values()].reduce(
        (a, b) => a + b,
        0,
      );
    ...
    rowsFor.push([
      white(scheme),
      dim(DESC[scheme]!),
      String(totalOf('train')),
      String(totalOf('test')),
    ]);
  }
  ui.rule('VALIDATION SPLITS');
  ui.table(['Split', 'Tests generalisation to', 'Train', 'Test'], rowsFor, [
    'l',
    'l',
    'r',
    'r',
...
function runPolicies(): void {
  ...
  const probabilities = new Map(
    preds.map((p) => [p.attempt_id, p.proba as Record<Cause, number>]),
  );
  const rulePredictions = new Map(
    preds.map((p) => [p.attempt_id, p.rule_predicted]),
  );
  const ratio = Number(argValue('--cost-ratio') ?? DEFAULT_COST_RATIO);
  const threshold = stopThreshold(ratio);
  if (probabilities.size === 0)
    throw new Error('no predictions -- run eval/evaluate.py first');
  ...
  const rupeeRate = (rows: PolicyOutcome[]) => {
    ...
    return denom === 0
      ? 0
      : rows.filter((r) => r.recovered).reduce((a, r) => a + r.amount, 0) /
          denom;
  };
  const retriesPer = (rows: PolicyOutcome[]) =>
    rows.length === 0
      ? 0
      : rows.reduce((a, r) => a + r.retries_spent, 0) / rows.length;

  ui.rule('RECOVERY POLICY COMPARISON');
  ui.note(
    `${failures.length.toLocaleString('en-IN')} failed payments · ${ui.money(totalAmount)} at risk · retry budget 3 per failure`,
  );
  ui.note(
    `stop rule: P(C4) ≥ ${threshold.toFixed(3)}  ·  cost ratio ${ratio}:1`,
  );
  ui.note(
    dim('a wrongful stop forfeits a mandate; a wrongful retry costs one retry'),
  );
  ...
  ui.table(
    ['P(C4) ≥', '₹ recovered', 'Retries', 'Net of retry cost', ''],
    sweep.map((r) => {
  ...
  ui.note(
    `sweep peaks at ${best.threshold.toFixed(2)}; cost model selects ${threshold.toFixed(3)}`,
  );
  ...
  ui.table(
    ['Cost / retry', 'Stops below', '₹ recovered', 'Retries', '₹ per retry'],
    evSweep.map((e) => {
      ...
      return [
        ...
        tone(ui.money(e.amount / e.retries)) +
          (chosen ? key('  ← in use') : ''),
      ];
    }),
    ['r', 'r', 'r', 'r', 'l'],
  ...
  for (const [name, schedule] of Object.entries(
    schedulesFor(probabilities, rulePredictions, threshold),
  )) {
  ...
  ui.table(
    ['Policy', 'Recovered', 'Share of at-risk ₹', 'Retries'],
    tableRows,
    ['l', 'r', 'l', 'r'],
  );
  ...
  for (const rival of [
    ...
    const d = pairedDeltaCI(
      outcomes.model_policy!,
      outcomes[rival]!,
      rupeeRate,
    );

    const rd = pairedDeltaCI(
      outcomes.model_policy!,
      outcomes[rival]!,
      retriesPer,
    );
  ...
  ui.table(['Compared with', 'Delta', '95% CI', 'Verdict'], deltaRows, [
    'l',
    'r',
    'l',
    'l',
  ...
  ui.table(
    ...
    ['naive_retry', 'window_aware_retry', 'rule_policy', 'oracle_policy'].map(
      (rival) => {
  ...
  ui.table(['Variant', 'Δ ₹', '95% CI', 'Δ retries', '95% CI'], evRows, [
    'l',
    'r',
    'l',
    'r',
  ...
  for (const ev of ['model_ev', 'model_ev_budget'] as const) {
    ...
    ui.line(
      `  ${POLICY_LABEL[ev]}: gives up ${ui.money(rupeesLost)} to save ` +
        ...
        (worth ? good : bad)(
          `worth it only above ${ui.money(breakeven)}/retry`,
        ),
    );
  }
  ui.note(
    `assumed retry cost is ${ui.money(COST_RETRY_RUPEES)} — set WHYDUNIT_RETRY_COST to your own`,
  );
  ...
  const silent = new Set(
    failures
      .filter(
        (f) => f.cause === 'C4_CANCELLATION' && !f.world.churn_emits_event,
      )
      .map((f) => f.attempt_id),
  );
  ...
  for (const cause of [
    ...
    const pick = (rows: PolicyOutcome[]) =>
      rows.filter((r) => causeOf.get(r.attempt_id) === cause);
  ...
  ui.table(['Cause', 'Naive', 'WhyDunit', 'Retries'], causeRows, [
    'l',
    'r',
    'r',
    'r',
  ]);
  const wasted = outcomes
    .model_policy!.filter((r) => silent.has(r.attempt_id))
    .reduce((a, r) => a + r.retries_spent, 0);
  ui.note('naive retry recovers 0% of C1: T+24/72/168h all keep the same hour');
  ui.note(
    `${wasted} retries burned on silent churn — unrecoverable, undetected`,
  );
    ...
    JSON.stringify(
      {
        ...
        sweep,
        ev_sweep: evSweep,
        retry_cost: COST_RETRY_RUPEES,
        results,
        paired_deltas: deltas,
        paired_retry_deltas: retryDeltas,
      },
      null,
      2,
    ) + '\n',
...
async function runAgentCommand(): Promise<void> {
  ...
  const observations = new Map(
    readJsonl<ObservedAttempt>('data/observations.jsonl').map((o) => [
      o.attempt_id,
      o,
    ]),
  );
  const preds = new Map(
    readJsonl<Prediction>('data/predictions.jsonl').map((p) => [
      p.attempt_id,
      p,
    ]),
  );
  if (!existsSync('data/exceptions.jsonl')) {
    throw new Error(
      'data/exceptions.jsonl missing -- run `npm run report` before the agent',
    );
  }
  const routed = new Set(
    readJsonl<{ attempt_id: string }>('data/exceptions.jsonl').map(
      (e) => e.attempt_id,
    ),
  );
  ...
  ui.note(
    `${work.length.toLocaleString('en-IN')} failed payments · ${resuming ? 'resuming' : 'fresh'} · ${dbPath}`,
  );
  ...
  if (s.resumed > 0)
    ui.step(
      'warn',
      `resumed ${s.resumed} in-flight intervention(s) from a previous run`,
    );
  ui.step('ok', 'cause-matched action chosen for every payment');
  ui.step('ok', 'four safety rules enforced before any action');
  ui.step(
    'ok',
    `${s.audit_rows.toLocaleString('en-IN')} audit records written`,
  );
  ui.step(
    'ok',
    `${s.psp_effects.toLocaleString('en-IN')} interventions sent, each exactly once`,
  );

  ui.rule('ACTIONS TAKEN');
  ui.table(
    ['Action', 'Payments', 'Share'],
    Object.entries(s.by_action)
      .sort((a, b) => b[1] - a[1])
      .map(([k, v]) => [
        white(ACTION_LABEL[k] ?? k),
        white(v.toLocaleString('en-IN')),
        ui.bar(v / s.audit_rows, 12) +
          dim(` ${((100 * v) / s.audit_rows).toFixed(1)}%`),
      ]),
    ['l', 'r', 'l'],
  );
  ...
  ui.table(
    ['Outcome', 'Mandates', ''],
    Object.entries(s.by_cycle_status)
      .sort((a, b) => b[1] - a[1])
      .map(([k, v]) => {
        const tone = k === 'recovered' ? good : k === 'escalated' ? warn : dim;
        return [
          tone(CYCLE_LABEL[k] ?? k),
          tone(v.toLocaleString('en-IN')),
          ui.bar(v / s.audit_rows, 12, tone),
        ];
      }),
    ['l', 'r', 'l'],
  );
  ...
  ui.kv([
    [
      'Recovered this run',
      good(ui.moneyShort(recovered)) + dim('  ' + ui.money(recovered)),
    ],
  ]);
  void AP;
}
...
async function runReport(): Promise<void> {
  ...
  const support = JSON.parse(
    readFileSync('data/support.json', 'utf8'),
  ) as Support;
  const world = new Map(
    readJsonl<WorldRecord>('data/world.jsonl').map((w) => [w.attempt_id, w]),
  );
  ...
  ui.line(
    '  ' +
      white(
        `${report.n_failures.toLocaleString('en-IN')} failed payments analyzed`,
      ),
  );
  ui.blank();
  ui.line(
    `  ${ui.OK} ${good(ui.pct(report.n_classified / report.n_failures))} ${white('Automatically Classified')}`,
  );
  ui.line(
    `  ${ui.WARN} ${warn(ui.pct(report.n_routed / report.n_failures))} ${white('Sent to Human Review')}`,
  );
  ui.blank();
  ui.note(
    'human review = low confidence, conflicting evidence, or thin history',
  );
  ...
  ui.kv([
    [
      'macro-F1 on auto-classified',
      ui.withCI(
        good(report.macro_f1_classified.toFixed(3)),
        report.macro_f1_classified_ci[0],
        report.macro_f1_classified_ci[1],
      ),
    ],
...
function renderCauseTable(report: Report): void {
  ...
  for (const cause of [
    ...
    rows.push([
      ...
      ui.pct(b.n / report.n_failures).padStart(5) +
        ' ' +
        ui.bar(b.n / report.n_failures, 12),
    ]);
  }
  rows.push([
    warn('?   Human Review'),
    warn(report.n_routed.toLocaleString('en-IN')),
    ui.pct(report.n_routed / report.n_failures).padStart(5) +
      ' ' +
      ui.bar(report.n_routed / report.n_failures, 12, warn),
  ]);
...
function renderQueueTable(report: Report): void {
  const rows = Object.entries(report.by_reason)
    .sort((a, b) => b[1] - a[1])
    .map(([k, v]) => [
      white(REASON_LABEL[k] ?? k),
      warn(v.toLocaleString('en-IN')),
    ]);
...
async function runDigest(): Promise<void> {
  ...
  if (!existsSync('data/report.json'))
    throw new Error('data/report.json missing -- run `npm run report` first');
  if (!existsSync('data/agent.db'))
    throw new Error(
      'data/agent.db missing -- run `npm run agent` before the digest',
    );
  ...
  if (!process.argv.includes('--explain')) {
    console.log(
      `${DP} natural-language layer skipped (pass --explain to enable)`,
    );
    return;
  }
  if (!hasCredentials()) {
    console.log(
      `${DP} --explain requested but no ANTHROPIC_API_KEY / ANTHROPIC_AUTH_TOKEN found; skipping`,
    );
    return;
  }
  ...
  const rows = new Map(
    readJsonl<FeatureFile>('data/features.jsonl').map((r) => [r.attempt_id, r]),
  );
      ...
      return {
        ...
        evidence:
          ev.length > 0
            ? ev
            : [
                `decline code ${w.error_code ?? 'none'}; no single observable is decisive`,
              ],
        action_taken:
          outcomes.action.get(r.mandate_id) ?? 'no intervention recorded',
  ...
  const excText = await explainExceptions(
    report.exceptions.slice(0, 20),
    explain,
  );
  writeJsonl('data/explanations.jsonl', [...attrText, ...excText]);
  writeFileSync(
    'data/digest.md',
    (await explainDigest(report, digest, explain)) + '\n',
  );
  console.log(
    `${DP} wrote data/explanations.jsonl (${attrText.length + excText.length}) and data/digest.md`,
  );
}
...
function readAgentOutcomes() {
  ...
  for (const r of db
    .prepare('SELECT outcome k, COUNT(*) n FROM audit_log GROUP BY outcome')
  ...
  for (const r of db
    .prepare(
      'SELECT mandate_id, source_attempt, cause, action, outcome FROM audit_log ORDER BY attempt_no',
    )
    .all() as {
    mandate_id: string;
...
function renderHealthPanel(report: Report): void {
  ...
  ui.kv([
    ...
    [
      'Money at risk',
      bad(ui.moneyShort(report.amount_at_risk)) +
        dim(`  ${ui.money(report.amount_at_risk)}`),
    ],
    [
      'Auto classified',
      good(ui.pct(report.n_classified / report.n_failures)) +
        dim(`  ${report.n_classified.toLocaleString('en-IN')}`),
    ],
    [
      'Human review',
      warn(ui.pct(report.n_routed / report.n_failures)) +
        dim(`  ${report.n_routed.toLocaleString('en-IN')}`),
    ],
  ]);
}
...
function renderDecisionTable(): void {
  ...
  rows.push([
    warn('Uncertain'),
    warn('HUMAN REVIEW'),
    dim('queued with evidence'),
  ]);
  ui.table(['Failure cause', 'Action', 'What WhyDunit does'], rows, [
    'l',
    'l',
    'l',
  ]);
  ...
  ui.table(
    ...
    Object.entries(tally)
      ...
      .map(([k, v]) => {
        ...
        return [
          tone(OUTCOME_LABEL[k] ?? k),
          tone(v.toLocaleString('en-IN')),
          ui.bar(v / total, 12, tone),
        ];
      }),
    ['l', 'r', 'l'],
  );
...
function renderRevenueHero(
  ...
  ui.kv([
    ...
    [
      'Money recovered',
      good(ui.moneyShort(recoveredAmount)) +
        dim('  ' + ui.money(recoveredAmount)),
    ],
  ]);
  ui.table(
    ...
    [
      [
        dim('Naive retry'),
        dim(ui.pct(naive.rate)),
        dim(naive.retries.toFixed(2)),
      ],
      [
        key('WhyDunit'),
        good(ui.pct(model.rate)),
        good(model.retries.toFixed(2)),
      ],
    ],
    ['l', 'r', 'r'],
  );
  ...
  ui.line(
    '  ' +
      v.tone(
        `${delta.delta >= 0 ? '+' : ''}${(100 * delta.delta).toFixed(1)}pp`,
      ) +
      ' recovery vs naive retry ' +
      dim(
        `[${(100 * delta.ci[0]).toFixed(1)}, ${(100 * delta.ci[1]).toFixed(1)}]`,
      ) +
      '  ' +
      v.tone(v.label),
  );
...
function recoveredAmountFrom(db: Database.Database): number {
  ...
  for (const w of readJsonl<WorldRecord>('data/world.jsonl'))
    amounts.set(w.mandate_id, w.amount);
  const rows = db
    .prepare("SELECT mandate_id FROM audit_log WHERE outcome='recovered'")
    .all() as { mandate_id: string }[];
...
function runDemo(): void {
  for (const f of ['data/report.json', 'data/policy.json', 'data/agent.db']) {
    if (!existsSync(f))
      throw new Error(`${f} missing -- run \`npm run all\` first`);
  }
  const report = JSON.parse(readFileSync('data/report.json', 'utf8')) as Report;
  const policy = JSON.parse(
    readFileSync('data/policy.json', 'utf8'),
  ) as PolicyFile;
  const db = new Database('data/agent.db', { readonly: true });
  const tally: Record<string, number> = {};
  for (const r of db
  ...
  ui.blank();
  const left =
    '  ' + ui.head('WHYDUNIT') + dim('  ·  AI Payment Recovery Intelligence');
  ui.line(
    left +
      ' '.repeat(Math.max(1, ui.W - ui.vlen(left) - 12)) +
      dim('RAZORPAY LAB'),
  );
  ...
  ui.kv([
    ...
    [
      'Naive retry recovery',
      dim(`${(100 * policy.results.naive_retry!.rate).toFixed(1)}%`),
    ],
    [
      'WhyDunit recovery',
      good(`${(100 * policy.results.model_policy!.rate).toFixed(1)}%`),
    ],
    [
      'Retries / failure',
      white(`${policy.results.naive_retry!.retries.toFixed(2)}`) +
        dim('  →  ') +
        good(`${policy.results.model_policy!.retries.toFixed(2)}`),
    ],
  ]);
  ui.blank();
...
function runExplainCase(): void {
  ...
  if (id === undefined)
    throw new Error('usage: node src/cli.ts explain <mandate_id>');
  for (const f of [
  ...
  if (attempts.length === 0)
    throw new Error(`no mandate ${id} in data/observations.jsonl`);

  const feats = new Map(
    readJsonl<FeatureRow>('data/features.jsonl').map((f) => [f.attempt_id, f]),
  );
  const preds = new Map(
    readJsonl<Prediction>('data/predictions.jsonl').map((p) => [
      p.attempt_id,
  ...
  const excs = existsSync('data/exceptions.jsonl')
    ? new Map(
        readJsonl<ExceptionFile>('data/exceptions.jsonl').map((e) => [
          e.attempt_id,
          e,
        ]),
      )
  ...
  if (failures.length === 0)
    throw new Error(`mandate ${id} never failed; nothing to attribute`);
  ...
  ui.line(
    '  ' +
      ui.head(`MANDATE ${id}`) +
      dim(`  ·  ${subject.bank}  ·  ${ui.money(subject.amount)} monthly`),
  );
  ui.line(dim('─'.repeat(ui.W)));

  ui.rule('1 · WHAT THE MERCHANT COULD SEE');
  const outcomes = attempts
    .map((a) => (a.success ? good('●') : bad('×')))
    .join(' ');
  ui.kv([
    ...
    [
      'This failure',
      `${subject.timestamp.slice(0, 16).replace('T', ' ')}  ${dim('code ' + (subject.error_code ?? 'none'))}`,
    ],
    [
      'Notice dispatched',
      `${subject.notification.hours_before_debit.toFixed(1)}h before debit`,
    ],
    [
      'Delivery receipt',
      subject.notification.receipt === null
        ? warn('none came back')
        : subject.notification.receipt === 'delivered'
          ? good('delivered')
          : bad('FAILED'),
    ],
    [
      'Revoke webhook',
      subject.lifecycle_events.length === 0
        ? dim('none')
        : warn(subject.lifecycle_events[0]!.timestamp.slice(0, 10)),
    ],
  ]);
  ui.blank();
  ...
  ui.table(
    ...
    [
      [
        'Hour of day',
        `${String(Number(subject.timestamp.slice(11, 13))).padStart(2, '0')} ${dim('· all: ' + failHours.map((h) => String(h).padStart(2, '0')).join(' '))}`,
        f.in_restricted_window === 1
          ? warn('in window')
          : varies(distinctHours > 1),
        f.in_restricted_window === 1
          ? key('C1 — inside 10:00-13:00')
          : distinctHours > 1
            ? dim('not C1 — many hours')
            : dim('not C1 — outside window'),
      ],
      [
        'Bank vs fleet',
        `${subject.bank} ${bankExcess >= 0 ? '+' : ''}${(100 * bankExcess).toFixed(1)}pp`,
        varies(Math.abs(bankExcess) > 0.05),
        Math.abs(bankExcess) > 0.05
          ? key('C2 — bank is an outlier')
          : dim('not C2 — bank normal'),
      ],
      [
        'Day of month',
        failDays.map((d) => String(d)).join(' '),
        varies(distinctDays > 1),
        Number(f.day_of_month ?? 0) >= 20
          ? key('C3 — late in cycle')
          : dim('early cycle — funds likely'),
      ],

      [
        'Recent run',
        `${f.consecutive_prior_failures ?? 0} in a row / ${f.prior_distinct_fail_hours ?? 0} hours`,
        varies(Number(f.consecutive_prior_failures ?? 0) < 2),
        Number(f.consecutive_prior_failures ?? 0) >= 2 &&
        Number(f.prior_distinct_fail_hours ?? 0) > 1
  ...
  ui.note(
    `outcome over time  ${attempts.map((a) => (a.success ? '▁' : '█')).join('')}   ${dim(`█ = failed · ${lifetime}/${attempts.length} ever succeeded`)}`,
  );

  ui.rule('3 · COMPETING HYPOTHESES');
  if (pred !== undefined) {
    const ranked = (Object.entries(pred.proba) as [Cause, number][]).sort(
      (a, b) => b[1] - a[1],
    );
    ui.table(
      ['Cause', 'P', '', 'Evidence'],
  ...
  if (exc !== undefined) {
    ...
    for (const rn of exc.reasons)
      ui.line(`  ${ui.WARN} ${warn(REASON_LABEL[rn] ?? rn)}`);
    ui.blank();
    ui.line('  ' + dim('what would resolve it:'));
    for (const rr of exc.resolving_evidence.slice(0, 3))
      ui.line(`    ${ui.ARROW} ${dim(rr)}`);
  } else if (pred !== undefined) {
    const conf = Math.max(...Object.values(pred.proba));
    ui.kv([
      [
        'Cause',
        key(`${CAUSE_CODE[pred.predicted]} — ${CAUSE_NAME[pred.predicted]}`),
      ],
  ...
  if (existsSync('data/agent.db')) {
    ...
    const audit = db
      .prepare(
        'SELECT * FROM audit_log WHERE mandate_id = ? ORDER BY cycle, attempt_no',
      )
      .all(id) as {
      attempt_no: number;
      cycle: string;
    ...
    if (audit.length === 0) {
      ...
      ui.blank();
      for (const chk of JSON.parse(last.checks_passed) as string[])
        ui.line(`  ${ui.OK} ${dim(CHECK_LABEL[chk] ?? chk)}`);
      for (const chk of JSON.parse(last.checks_failed) as string[])
        ui.line(`  ${ui.FAIL} ${bad(CHECK_LABEL[chk] ?? chk)}`);
      for (const chk of JSON.parse(last.checks_skipped) as string[])
        ui.line(`  ${dim('–')} ${dim((CHECK_LABEL[chk] ?? chk) + ' (n/a)')}`);

      ui.rule('6 · OUTCOME');
      ui.table(
        ['Cycle', '#', 'Action', 'Scheduled', 'Result'],
        audit.map((a) => [
          dim(a.cycle),
          dim(String(a.attempt_no)),
          ...
          (a.outcome === 'recovered'
            ...
            : a.outcome === 'failed'
              ? bad
              : dim)(OUTCOME_LABEL[a.outcome] ?? a.outcome),
        ]),
        ['l', 'r', 'l', 'l', 'l'],
      );
    }
  ...
  const truth = readJsonl<WorldRecord>('data/world.jsonl').find(
    (w) => w.attempt_id === subject.attempt_id,
  );
  if (truth === undefined) {
  ...
  ui.kv([
    [
      'True cause',
      white(`${CAUSE_CODE[truth.cause!]} — ${CAUSE_NAME[truth.cause!]}`),
    ],
    [
      'Attributed',
      exc !== undefined
        ? warn('routed to human')
        : pred === undefined
          ? dim('n/a')
          : (correct ? good : bad)(
              `${CAUSE_CODE[pred.predicted]} — ${correct ? 'correct' : 'WRONG'}`,
            ),
    ],
    [
      'Also blocked by',
    ...
    [
      'Cancelled at',
      truth.world.churned_at === null
        ? dim('never')
        : warn(
            truth.world.churned_at.slice(0, 10) +
              (truth.world.churn_emits_event ? ' (webhook sent)' : ' (SILENT)'),
          ),
    ],
  ]);
  ui.blank();
...
function headlineOf(): Record<string, number> {
  ...
  const evaluation = JSON.parse(
    readFileSync('data/evaluation.json', 'utf8'),
  ) as {
...
function runVerify(): void {
  ...
  if (!existsSync(MANIFEST))
    throw new Error(
      `${MANIFEST} missing -- run \`npm run verify -- --update\` first`,
    );
  ...
  ui.kv([
    [
      'Reference',
      dim(
        `seed ${want.config.seed} · horizon ${want.config.horizon_days}d · cost ratio ${want.config.cost_ratio}`,
      ),
    ],
    [
      'This run',
      dim(
        `seed ${SEED} · horizon ${HORIZON_DAYS}d · cost ratio ${DEFAULT_COST_RATIO}`,
      ),
    ],
  ]);
  let failed = 0;
  ...
  for (const [k, v] of Object.entries(want.config)) {
    ...
    if (now !== v) {
      ...
      ui.blank();
      ui.line(
        `  ${ui.FAIL} ${bad(`config ${k} is ${now}, manifest expects ${v}`)}`,
      );
      ui.note(
        "regenerate with the manifest's config, or re-run with --update if this is intended",
      );
    }
  }
  ...
  doneGen(
    `regenerated ${world.length.toLocaleString('en-IN')} attempts from seed`,
  );
  ...
  if (!sameWorld) failed++;
  ui.line(
    `  ${sameWorld ? ui.OK : ui.FAIL} ${sameWorld ? 'world matches the committed data byte for byte' : bad('regenerated world differs from data/world.jsonl')}`,
  );
  void observations;

  const unrounded: string[] = [];
  for (const line of readFileSync('data/predictions.jsonl', 'utf8')
    .trim()
    .split('\n')) {
    ...
    for (const [cause, p] of Object.entries(row.proba)) {
      if (Number(p.toFixed(6)) !== p)
        unrounded.push(`${row.attempt_id}.${cause}=${p}`);
    }
  }
  if (unrounded.length > 0) {
    failed++;
    ui.line(
      `  ${ui.FAIL} ${bad(`${unrounded.length} probabilities carry more than 6 dp — not portable across platforms`)}`,
    );
    ui.note(`first: ${unrounded[0]}`);
  } else {
    ui.line(
      `  ${ui.OK} probabilities are rounded at the serialization boundary (portable)`,
    );
  }
  ...
  for (const f of VERIFIED) {
    ...
    rows.push([
      f.replace('data/', ''),
      ok ? good('match') : bad('DIFFERS'),
      dim(actual.slice(0, 12)),
    ]);
  }
  ...
  if (moved.length > 0)
    ui.table(['Metric', 'Expected', 'Actual'], moved, ['l', 'r', 'r']);
  ...
  if (failed === 0) {
    ui.line(
      '  ' +
        good('✓ REPRODUCIBLE') +
        dim('  every artifact and headline metric matches the manifest'),
    );
  } else {
    ui.line(
      '  ' +
        bad(`✗ ${failed} MISMATCH${failed === 1 ? '' : 'ES'}`) +
        dim('  this run does not reproduce the committed reference'),
    );
    process.exitCode = 1;
  }
  ui.blank();
...
function main(): void {
  ...
  if (command !== 'generate') {
    console.error(
      'usage: node src/cli.ts <generate|features|report|policy|agent|digest|explain <mandate_id>|verify|demo>',
    );
    process.exit(1);
  }
  ...
  ui.note(
    `wrote data/world.jsonl (${world.length} records, ground truth included)`,
  );
  ui.note(
    `wrote data/observations.jsonl (${observations.length} records, merchant-visible only)`,
  );
