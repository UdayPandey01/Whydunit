...
export function indicators(
  ...
  if (f.in_restricted_window === 1) {
    out.C1_EXECUTION_WINDOW.push(
      `attempted at ${String(f.hour).padStart(2, '0')}:00 IST, inside the NPCI 10:00-13:00 restricted window`,
    );
  }
  if (f.receipt_delivered === 0) {
    out.C2_NOTIFICATION_FAIL.push(
      'the PSP delivery receipt for the pre-debit notification came back FAILED',
    );
  }
  if (f.notify_lead_under_24 === 1) {
    out.C2_NOTIFICATION_FAIL.push(
      `notification dispatched ${Number(f.notify_lead_hours).toFixed(1)}h before the debit, under the 24h NPCI minimum`,
    );
  }
  if (f.revoked_before_attempt === 1) {
    out.C4_CANCELLATION.push(
      `a mandate.revoked webhook arrived ${Number(f.hours_since_revoke).toFixed(0)}h before this attempt`,
    );
  }
  if (f.code_Z9 === 1) {
...
function circumstantial(
  ...
  if (cause === 'C2_NOTIFICATION_FAIL') {
    if (f.receipt_missing === 1)
      ev.push(
        'no delivery receipt came back at all, so bank-side delivery is unobserved',
      );
    if (
      f.bank_burst_z !== null &&
      f.bank_burst_z !== undefined &&
      f.bank_burst_z > 2
    ) {
      ev.push(
        `this bank's 24h failure rate is ${Number(f.bank_burst_z).toFixed(1)}σ above its own 30d baseline, consistent with an outage burst`,
      );
    }
  }
  if (cause === 'C3_BALANCE_SHORTFALL' && f.day_of_month !== null) {
    ev.push(
      `day ${f.day_of_month} of the month, ${f.mandate_prior_n} prior attempt(s) on this mandate`,
    );
  }
  if (cause === 'C4_CANCELLATION') {
    if (f.all_prior_failed === 1 && Number(f.prior_distinct_fail_hours) > 1) {
      ev.push(
        `${f.consecutive_prior_failures} consecutive failures across ${f.prior_distinct_fail_hours} distinct hours — no situational factor explains them`,
      );
    }
    if (f.mandate_prior_n === 0)
      ev.push(
        'no prior attempts, so failure invariance cannot be assessed at all',
      );
  }
...
const ABSENT: Record<Cause, (f: Record<string, number | null>) => string> = {
  ...
  C4_CANCELLATION: () =>
    'no revoke webhook, and no invariant run of failures to point at cancellation',
};
...
function resolvingEvidence(top: Cause[], reasons: ExceptionReason[]): string[] {
  ...
  out.push(
    ...(RESOLVES[key] ?? [
      'a further attempt under changed conditions is needed to separate the hypotheses',
    ]),
  );
  if (reasons.includes('insufficient_history')) {
    out.push(
      `at least ${MIN_PRIOR_ATTEMPTS} prior attempts on this mandate, so that failure invariance can be assessed`,
    );
  }
  if (reasons.includes('outside_training_support')) {
    out.push(
      'labelled examples from this bank or this operating regime, to bring it inside training support',
    );
  }
  return out;
}
...
export function routeException(
  ...
  const ranked = (Object.entries(probabilities) as [Cause, number][]).sort(
    (a, b) => b[1] - a[1],
  );
  ...
  if (Number(f.mandate_prior_n) < MIN_PRIOR_ATTEMPTS && !decisive) {
    ...
    detail.push(
      `${f.mandate_prior_n} prior attempt(s), and no single observable settles the cause`,
    );
  }
  ...
  if (margin < AMBIGUITY_MARGIN) {
    ...
    detail.push(
      `top two hypotheses separated by only ${margin.toFixed(3)} (${LABEL[top![0]]} ${top![1].toFixed(2)} vs ${LABEL[second![0]]} ${second![1].toFixed(2)})`,
    );
  }
  ...
  if (fired.length >= 2) {
    ...
    detail.push(
      `${fired.length} independent mechanisms show observable evidence at once: ${fired.map((c) => LABEL[c]).join(' and ')}`,
    );
  }
  ...
  if (!support.banks.includes(row.bank) || bankCount < MIN_BANK_TRAIN_ROWS) {
    ...
    if (novel.length >= NOVEL_FEATURE_THRESHOLD) {
      if (!reasons.includes('outside_training_support'))
        reasons.push('outside_training_support');
      detail.push(
        `${novel.length} features fall outside their training range (${novel
          .slice(0, 3)
          .map(([n]) => n)
          .join(', ')})`,
      );
  ...
  return {
    ...
    detail,
    hypotheses,
    resolving_evidence: resolvingEvidence(
      ranked.map(([c]) => c),
      reasons,
    ),
    model_said: top![0],
