...
import { explainAttributions, explainDigest, explainExceptions } from "../src/explain.ts";
...
function fixture(): { scored: Scored[]; world: Map<string, number> } {
  ...
  const scored = computeFeatures(observations).map((row) => {
    ...
    const predicted = (Object.entries(proba) as [Cause, number][]).sort((a, b) => b[1] - a[1])[0]![0];
    return { row, label, predicted, proba, amount: amounts.get(row.attempt_id)! };
  });
...
test("nothing in the scoring path imports the LLM layer", () => {
  for (const file of ["../src/exceptions.ts", "../src/report.ts", "../src/agent/agent.ts", "../src/features.ts"]) {
    const specifiers = importsOf(readFileSync(new URL(file, import.meta.url), "utf8"));
    for (const spec of specifiers) {
      assert.ok(!spec.includes("explain"), `${file} imports the explanation layer (${spec})`);
      assert.ok(!spec.includes("anthropic"), `${file} imports the Anthropic SDK (${spec})`);
    }
  }
});

test("the Anthropic SDK is imported in exactly one file", () => {
  const users = ["../src/exceptions.ts", "../src/report.ts", "../src/agent/agent.ts", "../src/features.ts",
    "../src/policy.ts", "../src/cli.ts", "../src/explain.ts"]
    .filter((f) => importsOf(readFileSync(new URL(f, import.meta.url), "utf8"))
      .some((spec) => spec.includes("anthropic")));
...
test("attribution and routing are byte-identical with and without explanations", async () => {
  ...
  const attributions: Attribution[] = scored.slice(0, 25).map((s) => ({
    ...
    timestamp: s.row.timestamp, amount: s.amount, cause: s.predicted,
    confidence: s.proba[s.predicted], evidence: ["synthetic"],
    action_taken: "reschedule", outcome: "failed",
  }));
...
test("explanations are strings only, and are never parsed back into a decision", async () => {
  const out = await explainAttributions(
    [{
      attempt_id: "att_1", mandate_id: "mdt_1", bank: "HDFC", timestamp: "2026-01-06T14:00:00+05:30",
      amount: 499, cause: "C3_BALANCE_SHORTFALL", confidence: 0.9, evidence: ["Z9"],
      action_taken: "reschedule", outcome: "recovered",
    }],
    ADVERSARIAL,
  );
