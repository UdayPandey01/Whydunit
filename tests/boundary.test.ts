...
test('no observation record contains any hidden key', () => {
  for (const o of observations) {
    ...
    for (const hidden of HIDDEN_KEYS) {
      assert.ok(
        !keys.has(hidden),
        `leaked hidden key ${hidden} in ${o.attempt_id}`,
      );
    }
  }
});
...
test('silent churn is invisible: revoked events only ever come from emitters', () => {
  ...
  assert.ok(
    silent.size > 0,
    'fixture should contain silent churn to be meaningful',
  );
  const visibleMandates = new Set(visible.map((o) => o.mandate_id));
  for (const m of silent)
    assert.ok(!visibleMandates.has(m), `silent churn ${m} became visible`);
});
