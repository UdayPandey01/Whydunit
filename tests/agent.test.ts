...
test('constraint: max 3 interventions per mandate per cycle', () => {
  const r = checkConstraints(basePlan, {
    ...okCtx,
    interventions_used: MAX_INTERVENTIONS_PER_CYCLE,
  });
...
test('constraint: never schedule inside the restricted window', () => {
  const r = checkConstraints(
    { ...basePlan, scheduled_at: istMs(2026, 0, 8, 11, 30) },
    okCtx,
  );
  assert.equal(r.ok, false);
  ...
  const r = checkConstraints(
    {
      ...
      notification_dispatch_at: scheduled - 23 * HOUR_MS,
    },
    okCtx,
  );
  assert.equal(r.ok, false);
  ...
  const edge = checkConstraints(
    {
      ...basePlan,
      scheduled_at: scheduled,
      notification_dispatch_at: scheduled - NOTIFY_MIN_LEAD_HOURS * HOUR_MS,
    },
    okCtx,
  );
...
test('constraint: never retry after a C4 determination or an explicit revoke', () => {
  ...
  const revoked = checkConstraints(basePlan, {
    ...okCtx,
    revoked_at: istMs(2026, 0, 6, 0, 0),
  });
...
test('checks are recorded as skipped, never silently passed', () => {
  const stop = checkConstraints(
    { ...basePlan, action: 'stop', scheduled_at: null },
    okCtx,
  );
  ...
  assert.deepEqual(
    noTime.skipped.sort(),
    [CHECKS.NOT_RESTRICTED, CHECKS.NOTIFY_LEAD].sort(),
  );
  assert.deepEqual(
    noTime.passed.sort(),
    [CHECKS.MAX_INTERVENTIONS, CHECKS.NOT_CANCELLED].sort(),
  );
});

test('decide maps each cause to its matched action', () => {
  const item = (
    cause: WorkItem['cause'],
    routed = false,
    pC4 = 0.01,
  ): WorkItem => ({
    source_attempt: 'a',
  ...
  assert.equal(
    act(item('C2_NOTIFICATION_FAIL'), 1, false),
    'refire_notification_then_reschedule',
  );
  assert.equal(act(item('C3_BALANCE_SHORTFALL'), 1, false), 'reschedule');
  assert.equal(
    act(item('C3_BALANCE_SHORTFALL'), 1, true),
    'stop',
    'an explicit revoke stops everything',
  );
  assert.equal(
    act(item('C3_BALANCE_SHORTFALL', true), 1, false),
    'escalate_to_human',
  );
...
test('stopping is cost-sensitive, not argmax', () => {
  const withC4 = (pC4: number): WorkItem => ({
    ...
    proba: {
      ...
      C3_BALANCE_SHORTFALL: 1 - pC4 - 0.02,
      C4_CANCELLATION: pC4,
    },
  });
  ...
  assert.equal(
    ...
    'must act on the best RETRYABLE cause',
  );

  assert.equal(decide(withC4(0.97), 1, false, t).action, 'stop');
  assert.equal(
    decide(withC4(0.951), 1, false, t).action,
    'reschedule',
    'just below the line keeps trying',
  );
...
const audit = db.prepare('SELECT * FROM audit_log').all() as Record<
  string,
  string | number | null
>[];
  ...
  for (const r of audit) {
    for (const field of [
      ...
      assert.notEqual(
        r[field],
        null,
        `${field} missing on ${r.idempotency_key}`,
      );
    }
    ...
    assert.equal(
      new Set(checks).size,
      4,
      `${r.idempotency_key} did not account for all 4 checks`,
    );
  }
});
...
test('no scheduled debit ever landed in the restricted window', () => {
  ...
  for (const r of scheduled) {
    assert.equal(
      isRestrictedTime(Date.parse(r.scheduled_at as string)),
      false,
...
test('no scheduled debit ever sat under 24h from its notification', () => {
  for (const r of audit.filter((x) => x.scheduled_at !== null)) {
    const lead =
      (Date.parse(r.scheduled_at as string) -
        Date.parse(r.notification_at as string)) /
      HOUR_MS;
    assert.ok(
      lead >= NOTIFY_MIN_LEAD_HOURS,
      `${r.idempotency_key} lead ${lead.toFixed(1)}h`,
    );
  }
});
...
test('no mandate exceeded its per-cycle intervention budget', () => {
  const used = db
    .prepare('SELECT mandate_id, cycle, interventions_used u FROM cycle_state')
    .all() as { mandate_id: string; cycle: string; u: number }[];
  assert.ok(used.length > 40);
  for (const r of used)
    assert.ok(
      r.u <= MAX_INTERVENTIONS_PER_CYCLE,
      `${r.mandate_id}/${r.cycle}=${r.u}`,
    );

  const perKey = new Map<string, number>();
  for (const r of audit) {
    if (r.outcome === 'blocked_by_constraint' || r.outcome === 'not_applicable')
      continue;
  ...
  for (const [k, n] of perKey)
    assert.ok(
      n <= MAX_INTERVENTIONS_PER_CYCLE,
      `${k} fired ${n} interventions`,
    );
});
...
test('nothing effectful ever happened after a cancellation', () => {
  const revoked = new Map(
    fixture.work
      .filter((w) => w.revoked_at !== null)
      .map((w) => [w.mandate_id, w.revoked_at!]),
  );
  ...
  for (const r of audit) {
    ...
    assert.notEqual(
      r.cause,
      'C4_CANCELLATION',
      `${r.idempotency_key} scheduled a retry on a C4`,
    );
    const rev = revoked.get(r.mandate_id as string);
    if (rev !== undefined) {
      examinedOnRevokedMandates++;
      assert.ok(
        Date.parse(r.scheduled_at as string) < rev,
        `${r.idempotency_key} fired after revoke`,
      );
    }
  }
  ...
  if (revoked.size > 0) {
    assert.ok(
      examinedOnRevokedMandates > 0,
      'no effectful action was examined on a revoked mandate',
    );
  }
});
  ...
  const r = checkConstraints(
    {
      ...basePlan,
      decided_at: istMs(2026, 0, 5, 9, 0),
      scheduled_at: istMs(2026, 0, 12, 14, 0),
    },
...
test('the PSP saw one effect per scheduled intervention, and no more', () => {
  const scheduled = audit.filter(
    (r) => r.scheduled_at !== null && r.outcome !== 'blocked_by_constraint',
  );
  const ledger = db.prepare('SELECT idempotency_key FROM psp_ledger').all() as {
    idempotency_key: string;
  }[];
  assert.equal(ledger.length, scheduled.length);
  assert.equal(
    new Set(ledger.map((l) => l.idempotency_key)).size,
    ledger.length,
  );
});
