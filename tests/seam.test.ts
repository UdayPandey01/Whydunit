...
class ScriptedPsp implements PspClient {
  ...
  async scheduleDebit(
    ...
    return this.succeedOn.has(key)
      ? OK(key)
      : FAILED('payment_declined', 'C3_BALANCE_SHORTFALL');
  }
...
function auditOf(dbPath: string) {
  ...
  const rows = db
    .prepare(
      'SELECT idempotency_key, action, cause, scheduled_at, checks_passed, checks_failed, status FROM audit_log ORDER BY idempotency_key',
    )
    .all();
  db.close();
  return rows;
...
test('the same agent cycle runs against two unrelated PSP implementations', async () => {
  ...
  const recovered = new Set(
    (
      dbSim
        .prepare(
          "SELECT idempotency_key FROM psp_ledger WHERE result='recovered'",
        )
  ...
  assert.ok(
    ...
    'the scripted PSP was never actually called',
  );
...
test('constraints still bind against a PSP that would accept anything', async () => {
  ...
  const db = new Database(dbPath, { readonly: true });
  const used = db
    .prepare('SELECT MAX(interventions_used) m FROM cycle_state')
    .get() as { m: number };
  db.close();
...
test('attribute / plan / execute each work standalone', async () => {
  ...
  assert.ok(
    plan.every((p) =>
      ['reschedule', 'refire_notification_then_reschedule', 'stop'].includes(
        p.action,
      ),
    ),
  );
...
test('cost ratio changes the stop threshold, and the default rule never stops alone', async () => {
  ...
  for (const ratio of [5, 20, 40]) {
    ...
    assert.equal(
      plan.filter((p) => p.stop).length,
      0,
      `rule scorer stopped at ratio ${ratio}`,
    );
  }
});
...
test('webhook signatures are verified over the raw body', () => {
  ...
  const raw = Buffer.from(
    JSON.stringify({ event: 'subscription.cancelled', payload: {} }),
  );
  ...
  assert.ok(
    !verifySignature(Buffer.from(raw.toString() + ' '), good, secret),
...
test('razorpay events map to our types, and cancellation has no decline code', () => {
  ...
  assert.equal(
    charged.observation?.notification.receipt,
    null,
    'bank-side delivery is unobservable',
  );
});
...
test('the code map claims only what Razorpay documents', () => {
  ...
  assert.equal(
    evidenceFor('payment_declined'),
    null,
    'generic declines must stay ambiguous',
  );
  ...
  assert.ok(
    Object.values(CODE_MAP).filter((v) => v.evidence_for !== null).length <= 2,
...
test("a retry is judged against a notice that precedes it, not the mandate's last", async () => {
  ...
  assert.ok(
    Date.parse(last.notification_dispatched_at) >
      Date.parse(first.notification_dispatched_at) + 60 * 86400_000,
...
test('the agent recovers a substantial share against the simulator', async () => {
  ...
  const total = Object.values(summary.by_cycle_status).reduce(
    (a, b) => a + b,
    0,
  );
