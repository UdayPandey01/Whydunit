...
test('a revoke webhook arriving after the attempt stays invisible', () => {
  const late = observations.filter((o) => {
    ...
    return (
      e !== undefined &&
      Date.parse(e.timestamp) > Date.parse(o.timestamp) &&
      !o.success
    );
  });
  assert.ok(
    late.length > 0,
    'fixture needs post-attempt revokes to be meaningful',
  );
  const byId = new Map(rows.map((r) => [r.attempt_id, r.features]));
  for (const o of late) {
    assert.equal(
      byId.get(o.attempt_id)!.revoked_before_attempt,
      0,
      o.attempt_id,
    );
    assert.equal(
      byId.get(o.attempt_id)!.hours_since_revoke,
      null,
      o.attempt_id,
    );
  }
...
test('splits are deterministic, disjoint and non-degenerate', () => {
  ...
  for (const [m, sides] of byMandate)
    assert.equal(sides.size, 1, `mandate ${m} spans both sides`);
  ...
  for (const scheme of ['mandate', 'bank', 'time'] as const) {
    ...
    assert.ok(
      test_n > 50 && test_n < rows.length - 50,
      `${scheme} split degenerate: ${test_n}`,
    );
  }
});
