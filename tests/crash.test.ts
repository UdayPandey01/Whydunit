...
function run(dbPath: string, crashAfter: number) {
  return spawnSync(process.execPath, [CHILD, dbPath, String(crashAfter)], {
    encoding: 'utf8',
  });
}
...
function readDb(dbPath: string) {
  ...
  const ledger = db
    .prepare(
      'SELECT idempotency_key, mandate_id, result FROM psp_ledger ORDER BY idempotency_key',
    )
    .all() as Ledger;
  const audit = db
    .prepare(
      'SELECT idempotency_key, action, status, outcome FROM audit_log ORDER BY idempotency_key',
    )
    .all() as {
...
test('the uninterrupted baseline is non-trivial', () => {
  ...
  assert.ok(
    clean.ledger.length > 60,
    `only ${clean.ledger.length} PSP effects -- fixture too small to prove anything`,
  );
  assert.ok(
    clean.audit.length > clean.ledger.length,
    'expected non-effectful decisions too',
  );
});
...
test('kill -9 at any point never double-fires, and resume reaches the same state', () => {
  ...
  for (let k = 1; k <= CRASH_POINTS; k++) {
    ...
    assert.equal(
      resumed.status,
      0,
      `resume failed at crash point ${k}: ${resumed.stderr}`,
    );
    ...
    assert.deepEqual(
      after.ledger,
      clean.ledger,
      `ledger diverged after crash at point ${k}`,
    );
    assert.equal(
    ...
    assert.equal(
      pending.length,
      0,
      `${pending.length} rows still pending after crash at point ${k}`,
    );
    ...
    assert.deepEqual(
      after.audit,
      clean.audit,
      `audit diverged after crash at point ${k}`,
    );
...
test('a crash mid-run really does leave work unfinished', () => {
  ...
  assert.ok(
    mid.ledger.length < clean.ledger.length,
    'child completed everything before being killed',
  );
  assert.ok(
