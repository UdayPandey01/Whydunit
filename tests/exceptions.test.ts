...
import {
  AMBIGUITY_MARGIN, indicators, MIN_BANK_TRAIN_ROWS, routeException,
...
const SUPPORT: Support = {
  ...
  feature_range: { hour: [0, 23], amount: [149, 4999], mandate_prior_n: [0, 2] },
};

function row(features: Record<string, number | null>, bank = "HDFC"): FeatureRow {
  return {
    attempt_id: "att_1", mandate_id: "mdt_1", bank, day_index: 5,
    timestamp: "2026-01-06T14:00:00+05:30",
    features: {
      mandate_prior_n: 2, in_restricted_window: 0, receipt_delivered: 1,
      notify_lead_under_24: 0, revoked_before_attempt: 0, code_Z9: 0,
      hour: 14, amount: 499, receipt_missing: 0, bank_burst_z: 0,
      day_of_month: 6, all_prior_failed: 0, consecutive_prior_failures: 0,
      prior_distinct_fail_hours: 0, hours_since_revoke: null, notify_lead_hours: 40,
      ...features,
    },
  };
}
const CONFIDENT: Record<Cause, number> = {
  C1_EXECUTION_WINDOW: 0.05, C2_NOTIFICATION_FAIL: 0.05,
  C3_BALANCE_SHORTFALL: 0.85, C4_CANCELLATION: 0.05,
};
...
test("rule: top-two probabilities within the margin", () => {
  const close: Record<Cause, number> = {
    C1_EXECUTION_WINDOW: 0.02, C2_NOTIFICATION_FAIL: 0.02,
    C3_BALANCE_SHORTFALL: 0.49, C4_CANCELLATION: 0.47,
  };
...
test("rule: multi-cause conflict, detected from observables only", () => {
  const r = routeException(
    row({ in_restricted_window: 1, hour: 11, receipt_delivered: 0 }), CONFIDENT, SUPPORT, 499,
  );
  assert.ok(r !== null);
  assert.ok(r.reasons.includes("multi_cause_conflict"));
  assert.ok(r.detail.some((d) => d.includes("execution window") && d.includes("notification")));
});
...
test("rule: bank outside training support", () => {
  ...
  const thin: Support = { ...SUPPORT, bank_train_counts: { HDFC: MIN_BANK_TRAIN_ROWS - 1, ICICI: 200 } };
...
test("every exception carries reason, competing hypotheses and resolving evidence", () => {
  const r = routeException(row({ mandate_prior_n: 0 }), CONFIDENT, SUPPORT, 499);
  assert.ok(r !== null);
  assert.ok(r.reasons.length > 0);
  assert.ok(r.detail.length > 0);
  assert.ok(r.hypotheses.length >= 2, "an exception needs competing hypotheses, not one");
  assert.ok(r.resolving_evidence.length > 0);
  for (const h of r.hypotheses) assert.ok(h.evidence.length > 0, `${h.cause} has no evidence line`);
});
...
test("the observable multi-cause detector never reaches for world state", () => {
  ...
  const src = readFileSync(new URL("../src/exceptions.ts", import.meta.url), "utf8");
  for (const banned of ["multi_cause", "balance_at_attempt", "churned_at", "notification_delivered_by_bank"]) {
    assert.ok(!src.includes(`.${banned}`), `exceptions.ts reads hidden world field ${banned}`);
  }
});
...
test("no exception record leaks the ground-truth label", () => {
  ...
  for (const banned of ["label", "actual", "cause", "multi_cause", "blockers"]) {
